        ...
        node_type: Type of node to create. Supported types (23 total):
            ...
            DATA:
                "VariableGet" - Read a variable value (⚠️ variable must exist in Blueprint)
                    ℹ️ Use target_class="/Script/Module.ClassName" to access UPROPERTY of external C++ class
...
def add_component_bound_event(
    ...
    """
    Создать узел ComponentBoundEvent для привязки делегата компонента (K2Node_ComponentBoundEvent).

    Используется для привязки событий UMG-виджетов — например, Button.OnClicked.
    Создаёт узел события в Event Graph Blueprint, который срабатывает при активации делегата компонента.

    Args:
        blueprint_name: Имя Blueprint (например, "WBP_DeploymentScreen")
        component_property_name: Имя UPROPERTY компонента в Blueprint (например, "ConfirmButton")
        delegate_name: Имя делегата на компоненте (например, "OnClicked")
        delegate_class: Полный путь класса-владельца делегата (например, "/Script/UMG.Button")
        pos_x: X позиция узла в графе (по умолчанию: 0)
        pos_y: Y позиция узла в графе (по умолчанию: 0)

    Returns:
        Словарь с node_id, delegate_name, component_property_name или ошибкой

    Пример использования:
        add_component_bound_event(
            blueprint_name="/Game/UI/WBP_DeploymentScreen",
            component_property_name="ConfirmButton",
    ...
    try:
        ...
        logger.error(f"add_component_bound_event error: {e}")
        return {"success": False, "message": str(e)}
...
def add_widget_to_umg(
    ...
    Args:
        blueprint_path: Full asset path, e.g. "/Game/UI/MainMenu/WBP_GameSettings"
        widget_type: Widget class name. Supported: CanvasPanel, TextBlock, Button,
                     VerticalBox, HorizontalBox, ScrollBox, SizeBox, Spacer,
                     SpinBox, Image, Overlay, Border, ProgressBar
        widget_name: Unique name for the widget inside this Blueprint
        parent_name: Name of the parent panel widget. If empty, adds to root or sets as root.
        is_variable: Whether to expose the widget as a Blueprint variable (default True)

    Returns:
        Dictionary with widget_name, widget_type, parent_name or error
    """
    unreal = get_unreal_connection()
    if not unreal:
        return {"success": False, "message": "Failed to connect to Unreal Engine"}
    ...
    try:
        ...
        logger.error(f"add_widget_to_umg error: {e}")
        return {"success": False, "message": str(e)}
...
def set_widget_property(
    ...
    Common properties:
      All widgets:  Visibility (Visible/Collapsed/Hidden), IsEnabled (true/false)
      TextBlock:    Text, FontSize (int), ColorAndOpacity ("R,G,B" or "R,G,B,A"),
                    Justification (Left/Center/Right), AutoWrapText (true/false)
      Button:       ColorAndOpacity, BackgroundColor ("R,G,B,A")
      Spacer:       Size ("X,Y")
      SizeBox:      WidthOverride, HeightOverride, MinDesiredWidth/Height, MaxDesiredWidth/Height
      Image:        ColorAndOpacity, Opacity
      ProgressBar:  Percent, FillColorAndOpacity
      Slot props:   Slot.Padding ("float" or "T,L,B,R"), Slot.HorizontalAlignment (Left/Center/Right/Fill),
                    Slot.VerticalAlignment (Top/Center/Bottom/Fill),
                    Slot.FillHeight / Slot.AutoHeight (VerticalBoxSlot),
                    Slot.FillWidth / Slot.AutoWidth (HorizontalBoxSlot),
                    Slot.Position, Slot.Size, Slot.Alignment (CanvasPanelSlot, as "X,Y")

    Args:
        blueprint_path: Full asset path, e.g. "/Game/UI/MainMenu/WBP_GameSettings"
        widget_name: Name of the widget to modify (use "Root" for root widget)
        property_name: Property to set (see above)
        property_value: Value as string

    Returns:
        Dictionary with widget_name, property_name, status or error
    """
    unreal = get_unreal_connection()
    if not unreal:
        return {"success": False, "message": "Failed to connect to Unreal Engine"}
...
@mcp.tool()
def get_umg_hierarchy(blueprint_path: str) -> Dict[str, Any]:
    ...
    Returns:
        Dictionary with 'root' containing the widget tree, or 'empty': true if no widgets
    """
    unreal = get_unreal_connection()
    if not unreal:
        return {"success": False, "message": "Failed to connect to Unreal Engine"}
    ...
    try:
        ...
        logger.error(f"get_umg_hierarchy error: {e}")
        return {"success": False, "message": str(e)}
